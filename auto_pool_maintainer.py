...
def build_cpa_base_url(port: int = 8317) -> str:
...
# 号池维护配置
# candidates 低于这个值时触发补号。
MAINTAINER_MIN_CANDIDATES = 6000
# loop 模式每轮检查间隔，单位秒。
MAINTAINER_LOOP_INTERVAL_SECONDS = 60.0
DEFAULT_LOOP_INTERVAL_SECONDS = MAINTAINER_LOOP_INTERVAL_SECONDS
# loop 模式允许的最小间隔，避免配置过小导致空转。
MIN_LOOP_INTERVAL_SECONDS = 5.0
# True 表示默认进入 loop 模式；可被 --loop / --once 覆盖。
LOOP_MODE_ENABLED = True
# True 表示先清理问题文件再补号；False 表示只补号不清理。
CLEAN_MODE_ENABLED = False

# 邮箱模式配置
# "config" 表示按 config.json 中的 mail.provider 走；
# "self_hosted_messages_api" 表示强制使用下面这套 /messages 接口。
...
def parse_cli_bool(raw: Any) -> bool:
    if isinstance(raw, bool):
        return raw
    text = str(raw or "").strip().lower()
    ...
    if text in {"0", "false", "no", "off", "n"}:
        return False
    raise argparse.ArgumentTypeError(f"不支持的布尔值: {raw!r}，请使用 true/false")
...
def setup_logger(log_dir: Path) -> tuple[logging.Logger, Optional[Path]]:
    ...
    setattr(logger, "flow_trace", None)
    return logger, None
...
def parse_mail_domains_csv(raw: Any) -> List[str]:
    text = str(raw or "").strip()
    if not text:
        return []
    parts = [item.strip() for item in text.split(",")]
    return normalize_mail_domains(parts)


def resolve_account_password(conf: Dict[str, Any]) -> str:
    fixed_password = str(pick_conf(conf, "run", "account_password", "password", default=FIXED_ACCOUNT_PASSWORD) or "").strip()
    return fixed_password or generate_random_password()
...
def normalize_mail_provider(value: str) -> str:
    ...
    aliases = {
        ...
        "messages_api": "self_hosted_messages_api",
        "main_messages_api": "self_hosted_messages_api",
        "self_hosted": "self_hosted_mail_api",
        "self_hosted_mail_api": "self_hosted_mail_api",
        "self_hosted_messages": "self_hosted_messages_api",
        "self_hosted_messages_api": "self_hosted_messages_api",
        "duckmail": "duckmail",
...
class SelfHostedMessagesApiProvider(DomainAwareMailProvider):
    provider_name = "self_hosted_messages_api"
    ...
    def __init__(
        ...
        *,
        ...
        self.domain = self.domains[0] if self.domains else ""
        if not self.api_url:
            raise RuntimeError("SELF_HOSTED_MESSAGES_API_URL 未配置，无法调用 messages 邮箱接口。")
        if not self.domains:
            raise RuntimeError("SELF_HOSTED_MESSAGES_DOMAINS 未配置，无法生成邮箱地址。")

    @staticmethod
    def _sort_key(item: Dict[str, Any]) -> tuple[str, int]:
        msg_id = str(item.get("id") or "").strip()
        try:
            msg_id_num = int(msg_id)
        except (TypeError, ValueError):
            msg_id_num = 0
        return (str(item.get("received_at") or ""), msg_id_num)
    ...
    def _fetch_messages(self, email: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if not isinstance(messages, list):
                return []
            return sorted(messages, key=self._sort_key, reverse=True)
        except Exception:
            return []
    ...
    def poll_verification_codes(
        ...
        *,
        ...
        messages = self._fetch_messages(mailbox.email)
        codes: List[str] = []
        for message in messages[:20]:
            message_id = str(message.get("id") or "").strip()
            signature = message_id or _mail_content_signature(message)
            if seen_ids is not None and signature:
    ...
    def describe(self) -> str:
        return f"{self.provider_name}({self.api_url}, domains={','.join(self.domains)})"
...
def build_mail_provider(conf: Dict[str, Any], proxy: str, logger: logging.Logger) -> MailProviderBase:
    configured_provider = str(pick_conf(conf, "mail", "provider", "mail_provider", default="") or "").strip()
    forced_provider = str(MAIL_PROVIDER_MODE or "").strip()
    raw_provider = configured_provider if forced_provider.lower() in {"", "config"} else forced_provider
    ...
    if provider_name == "self_hosted_messages_api":
        return SelfHostedMessagesApiProvider(
            proxy=proxy,
            logger=logger,
            api_url=SELF_HOSTED_MESSAGES_API_URL,
            domains=normalize_mail_domains(list(SELF_HOSTED_MESSAGES_DOMAINS)),
            failure_threshold=failure_threshold,
            failure_cooldown_seconds=failure_cooldown_seconds,
        )
...
class ProtocolRegistrar:
    ...
    def _capture_registration_tokens(
        ...
        self.logger.warning(
            "注册Token捕获: auth_code=%s code_verifier=%s callback_keys=%s has_tokens=%s token_keys=%s",
            bool(auth_code),
            bool(self.code_verifier),
            sorted((callback_params or {}).keys()),
            bool(self.registration_tokens),
            sorted((self.registration_tokens or {}).keys()) if isinstance(self.registration_tokens, dict) else [],
        )
    ...
    def _bootstrap_codex_authorize(self, client_id: str, redirect_uri: str) -> tuple[bool, str]:
    ...
    def _init_session_via_direct_auth(self, client_id: str, redirect_uri: str) -> tuple[bool, str]:
        def _do_init() -> tuple[bool, str]:
            return self._bootstrap_codex_authorize(client_id=client_id, redirect_uri=redirect_uri)

        return self._run_step_with_retry("0a_direct_auth", _do_init)

    def _init_session_via_chatgpt_web(self, client_id: str, redirect_uri: str) -> tuple[bool, str]:
        chatgpt_base = self.chatgpt_base
    ...
    def step0_init_oauth_session(self, email: str, client_id: str, redirect_uri: str) -> bool:
        ...
        for index, mode in enumerate(self._entry_mode_candidates(), start=1):
            if mode == "chatgpt_web":
                ok, reason = self._init_session_via_chatgpt_web(client_id=client_id, redirect_uri=redirect_uri)
            else:
    ...
    def register(
        ...
        self.code_verifier = None
        self.state = None
        self.oauth_client_id = str(client_id or "")
        self.oauth_redirect_uri = str(redirect_uri or "")
        self.registration_auth_code = ""
...
def codex_exchange_code(
    ...
    try:
        ...
        if resp.status_code == 200:
            data = resp.json()
            if logger and isinstance(data, dict) and not has_usable_auth_tokens(data):
                logger.warning(
                    "OAuth token exchange incomplete: label=%s status=%s keys=%s error=%s error_description=%s",
                    exchange_label or "-",
                    resp.status_code,
                    sorted(data.keys()),
                    str(data.get("error") or "").strip() or "-",
                    str(data.get("error_description") or "").strip() or "-",
                )
            return data if isinstance(data, dict) else None
        if logger:
            error = "-"
            ...
            if isinstance(payload, dict):
                ...
                error_description = str(payload.get("error_description") or "").strip() or "-"
            logger.warning(
                "OAuth token exchange failed: label=%s status=%s error=%s error_description=%s",
                exchange_label or "-",
                resp.status_code,
                error,
                error_description,
        ...
        if logger:
            logger.warning(
                "OAuth token exchange exception: label=%s error=%s",
                exchange_label or "-",
                error,
            )
        return None
...
def exchange_codex_tokens_from_session(
    ...
    return codex_exchange_code(
        ...
        proxy=proxy,
        logger=logger,
        exchange_label="consent_session",
    )


def exchange_codex_tokens_from_logged_in_session(
    session: requests.Session,
    oauth_issuer: str,
    oauth_client_id: str,
    oauth_redirect_uri: str,
    ...
    active_device_id = str(device_id or uuid.uuid4())
    session.cookies.set("oai-did", active_device_id, domain=".auth.openai.com")
    session.cookies.set("oai-did", active_device_id, domain="auth.openai.com")
    ...
    try:
        ...
        callback_params = extract_oauth_callback_params_from_response(
            {},
            response_headers=authorize_resp.headers,
            response_url=str(authorize_resp.url or ""),
            response_text=authorize_resp.text,
        )
        consent_url = extract_continue_url_from_response(
            {},
            response_headers=authorize_resp.headers,
            response_url=str(authorize_resp.url or ""),
        )
        if flow_trace is not None:
            flow_trace.record(
                "oauth_logged_in_session_authorize_response",
                response=build_response_trace_payload(
                    authorize_resp,
                    reveal_sensitive=flow_trace.reveal_sensitive,
                    body_limit=flow_trace.body_limit,
        ...
        match = re.search(r'(https?://localhost[^\s\'"]+)', str(error))
        if match:
            callback_params = extract_oauth_callback_params_from_url(match.group(1))
            consent_url = match.group(1)
    except Exception as error:
        if logger:
            logger.warning("已登录会话发起 Codex OAuth 失败: %s", error)
        return None

    if not callback_params and consent_url:
        callback_params = extract_oauth_callback_params_from_consent_session(
            session=session,
            consent_url=consent_url,
            oauth_issuer=oauth_issuer,
    ...
    if not auth_code:
        if logger:
            logger.warning("已登录会话未拿到 Codex OAuth code")
        return None

    tokens = codex_exchange_code(
        auth_code,
        code_verifier,
        oauth_issuer=oauth_issuer,
        oauth_client_id=oauth_client_id,
    ...
    if flow_trace is not None:
        flow_trace.record(
            "oauth_logged_in_session_complete",
            success=bool(tokens),
...
def find_value_in_data_by_keys(data: Any, keys: tuple[str, ...], max_depth: int = 5) -> str:
    normalized_keys = {str(key).strip().lower().replace("-", "_") for key in keys if str(key).strip()}
    if not normalized_keys:
        return ""

    def _search(node: Any, depth: int) -> str:
        if depth > max_depth:
            return ""
            ...
            for key, value in node.items():
                ...
                if normalized_key in normalized_keys:
                    if isinstance(value, str):
                        ...
                        if candidate:
                            return candidate
                candidate = _search(value, depth + 1)
                if candidate:
                    return candidate
            return ""
...
def has_usable_auth_tokens(tokens: Optional[Dict[str, Any]]) -> bool:
    if not isinstance(tokens, dict):
        return False
    return bool(str(tokens.get("access_token") or "").strip())


def missing_auth_token_keys(
    tokens: Optional[Dict[str, Any]],
    keys: tuple[str, ...] = ("access_token", "refresh_token", "id_token"),
) -> tuple[str, ...]:
    if not isinstance(tokens, dict):
        return keys
    return tuple(
...
def collect_nested_key_paths(
    data: Any,
    *,
    needle_parts: tuple[str, ...] = ("token", "auth", "session"),
    max_depth: int = 4,
    max_items: int = 40,
) -> list[str]:
    matches: list[str] = []
    ...
    def _walk(node: Any, path: str, depth: int) -> None:
        if depth > max_depth or len(matches) >= max_items:
            return
        if isinstance(node, dict):
            for key, value in node.items():
                ...
                if any(part in lowered for part in needle_parts):
                    matches.append(next_path)
                    if len(matches) >= max_items:
                        return
                _walk(value, next_path, depth + 1)
                if len(matches) >= max_items:
                    return
            return
        if isinstance(node, (list, tuple)):
            for index, item in enumerate(node):
                _walk(item, f"{path}[{index}]" if path else f"[{index}]", depth + 1)
                if len(matches) >= max_items:
                    return
...
def build_normalized_auth_token_result(
    access_token: str,
    refresh_token: str = "",
    id_token: str = "",
    email_hint: str = "",
    ...
    payload = decode_jwt_payload(normalized_access_token)
    id_payload = decode_jwt_payload(normalized_id_token) if normalized_id_token else {}
    auth_info = payload.get("https://api.openai.com/auth", {})
    account_id = extract_chatgpt_account_id(auth_info) if isinstance(auth_info, dict) else None
    email = str(payload.get("email") or id_payload.get("email") or email_hint or "").strip()
    exp = payload.get("exp")
    ...
    return {
        ...
        "email": email,
        "account_id": str(account_id or account_id_hint or ""),
...
def build_chatgpt_session_token_result(
    ...
    logged_in_oauth_tokens: Dict[str, Any] = {}
    if (
        effective_callback_params.get("code")
        and code_verifier
        and oauth_client_id
        ...
        oauth_tokens = (
            codex_exchange_code(
                effective_callback_params["code"],
                code_verifier,
                oauth_issuer=oauth_issuer,
                oauth_client_id=oauth_client_id,
                oauth_redirect_uri=oauth_redirect_uri,
                proxy=proxy,
                logger=logger,
                exchange_label="chatgpt_session_callback",
            )
            or {}
        )
        if active_trace is not None:
            active_trace.record(
                "chatgpt_session_oauth_exchange",
                success=bool(oauth_tokens),
                token_keys=sorted(oauth_tokens.keys()),
            )
    if has_usable_auth_tokens(oauth_tokens):
        access_token = str(oauth_tokens.get("access_token") or "").strip()
        refresh_token = str(oauth_tokens.get("refresh_token") or "").strip()
    ...
    if effective_callback_params.get("code"):
        ...
        try:
            ...
            if has_usable_auth_tokens(oauth_tokens):
                return build_normalized_auth_token_result(
                    access_token=str(oauth_tokens.get("access_token") or "").strip(),
                    refresh_token=str(oauth_tokens.get("refresh_token") or "").strip(),
                    id_token=str(oauth_tokens.get("id_token") or "").strip(),
                )
    ...
    if not access_token:
        access_token = find_value_in_data_by_keys(session_data, ("accessToken", "access_token"))
    if not access_token:
        access_token = find_jwt_in_data(session_data)
    if not access_token:
        return None

    session_refresh_token = find_value_in_data_by_keys(session_data, ("refreshToken", "refresh_token"))
    if not refresh_token:
        refresh_token = session_refresh_token
    if not refresh_token:
        refresh_token = str(oauth_tokens.get("refresh_token") or "").strip()

    session_id_token = find_value_in_data_by_keys(session_data, ("idToken", "id_token"))
    if not id_token:
        id_token = session_id_token
    if not id_token:
        id_token = str(oauth_tokens.get("id_token") or "").strip()

    if (not refresh_token or not id_token) and oauth_client_id and oauth_redirect_uri:
        logged_in_oauth_tokens = exchange_codex_tokens_from_logged_in_session(
            session=session,
            oauth_issuer=oauth_issuer,
            oauth_client_id=oauth_client_id,
            oauth_redirect_uri=oauth_redirect_uri,
        ...
        if has_usable_auth_tokens(logged_in_oauth_tokens):
            access_token = str(logged_in_oauth_tokens.get("access_token") or access_token).strip()
            refresh_token = str(logged_in_oauth_tokens.get("refresh_token") or "").strip()
            id_token = str(logged_in_oauth_tokens.get("id_token") or "").strip()
        if not refresh_token:
            refresh_token = str(logged_in_oauth_tokens.get("refresh_token") or "").strip()
        if not id_token:
            id_token = str(logged_in_oauth_tokens.get("id_token") or "").strip()
    ...
    if logger and (not refresh_token or not id_token):
        logger.warning(
            "Token诊断: missing=%s auth_code=%s callback_keys=%s oauth_keys=%s session_keys=%s user_keys=%s session_token_paths=%s logged_in_oauth_keys=%s",
            ",".join(missing_auth_token_keys({
            ...
            bool(auth_code),
            sorted((callback_params or {}).keys()),
            sorted(oauth_tokens.keys()),
            sorted(session_data.keys()) if isinstance(session_data, dict) else [],
            sorted(user_info.keys()) if isinstance(user_info, dict) else [],
            collect_nested_key_paths(session_data),
            sorted(logged_in_oauth_tokens.keys()),
        )
    return build_normalized_auth_token_result(
        access_token=access_token,
        refresh_token=refresh_token,
        id_token=id_token,
        email_hint=(user_info.get("email") if isinstance(user_info, dict) else "") or "",
    )
...
class RegisterRuntime:
    def __init__(self, conf: Dict[str, Any], target_tokens: int, logger: logging.Logger):
        ...
        self.concurrent_workers = max(1, REGISTER_WORKER_COUNT)
        ...
        self.cpa_manager = ZeaburAuthFileManager(CPA_BASE_URL, CPA_TOKEN)
        ...
        token_json_dir_raw = str(output_cfg.get("token_json_dir", "") or "").strip()

        self.run_dir = os.getcwd()
        self.cpa_upload_dir = os.path.join(self.run_dir, ".cpa_upload_tmp")
        os.makedirs(self.cpa_upload_dir, exist_ok=True)
        if self.save_local:
            ...
            if token_json_dir_raw:
                self.tokens_out_dir = self._resolve_output_dir(token_json_dir_raw)
                os.makedirs(self.tokens_out_dir, exist_ok=True)
            else:
    ...
    def _resolve_output_dir(self, value: str) -> str:
        normalized = os.path.expanduser(str(value or "").strip())
        if os.path.isabs(normalized):
            return normalized
        return os.path.join(self.run_dir, normalized)
    ...
    def _upload_via_cpa_manager(
        ...
        *,
        ...
        staged_name = os.path.basename(str(upload_name or "")).strip()
        if not staged_name:
            self.logger.warning("上传 token 失败: 缺少文件名")
            return False

        staged_path = os.path.join(self.cpa_upload_dir, staged_name)
        try:
            ensure_parent_dir(staged_path)
            if source_filename:
                with open(source_filename, "rb") as src, open(staged_path, "wb") as dst:
                    dst.write(src.read())
            else:
    ...
    def upload_token_json(self, filename: str) -> bool:
        return self._upload_via_cpa_manager(
...
def register_one(runtime: RegisterRuntime, worker_id: int = 0) -> tuple[Optional[str], Optional[bool], float, float]:
    ...
    password = resolve_account_password(runtime.conf)
    ...
    if flow_trace is not None:
        flow_trace.record(
            ...
            email=email,
            source="registration" if has_complete_auth_tokens(tokens) else ("registration_partial" if tokens else "oauth_retry"),
        )
    if not has_complete_auth_tokens(tokens):
        oauth_tokens = runtime.oauth_login_with_retry(mailbox=mailbox, password=password)
        if oauth_tokens:
            tokens = oauth_tokens
            if flow_trace is not None:
                flow_trace.record(
                    "register_one_token_source_upgrade",
                    email=email,
                    source="oauth_retry",
                    reason="missing_refresh_or_id_token" if registrar.registration_tokens else "registration_tokens_missing",
                )
    t_total = time.time() - t_start
    if not has_complete_auth_tokens(tokens):
        oauth_detail = runtime.last_oauth_failure_detail or f"attempts={runtime.oauth_outer_retry_attempts}"
        if tokens:
            oauth_detail = f"incomplete_tokens:{','.join(missing_auth_token_keys(tokens))}"
...
def run_batch_register(conf: Dict[str, Any], target_tokens: int, logger: logging.Logger) -> tuple[int, int, int]:
    ...
    if runtime.tokens_out_dir:
        logger.info("Token JSON 本地目录: %s", runtime.tokens_out_dir)
...
def resolve_loop_interval_seconds(conf: Dict[str, Any], cli_value: Optional[float] = None) -> float:
    ...
    if cli_value is not None:
        ...
        raw_value = MAINTAINER_LOOP_INTERVAL_SECONDS
...
def apply_runtime_overrides(conf: Dict[str, Any], args: argparse.Namespace) -> Dict[str, Any]:
    run_cfg = conf.get("run")
    if not isinstance(run_cfg, dict):
        run_cfg = {}
        conf["run"] = run_cfg

    mail_cfg = conf.get("mail")
    ...
    if not isinstance(output_cfg, dict):
        output_cfg = {}
        conf["output"] = output_cfg

    token_json_dir = str(getattr(args, "token_json_dir", "") or "").strip()
    if token_json_dir:
    ...
    if proxy_url:
        ...
        run_cfg["proxy"] = ""
    elif use_proxy is True and not existing_proxy:
        raise RuntimeError("--use-proxy=true 时必须同时提供 --proxy-url，或在 config.json 中预置 run.proxy")

    account_password = str(getattr(args, "account_password", "") or "").strip()
    if account_password:
        run_cfg["account_password"] = account_password

    mail_mode_raw = str(getattr(args, "mail_mode", "") or "").strip()
    if mail_mode_raw:
        normalized_provider = normalize_mail_provider(mail_mode_raw)
        if normalized_provider != "config":
            mail_cfg["provider"] = normalized_provider

    mail_api = str(getattr(args, "mail_api", "") or "").strip()
    if mail_api:
        mail_cfg["api_base"] = mail_api

    mail_domains = parse_mail_domains_csv(getattr(args, "mail_domains", ""))
    if mail_domains:
        mail_cfg["domains"] = mail_domains

    return conf


def apply_cli_global_overrides(args: argparse.Namespace) -> None:
    global CPA_BASE_URL
    global CPA_TOKEN
    ...
    cpa_base_url = str(getattr(args, "cpa_base_url", "") or "").strip()
    if cpa_base_url:
        CPA_BASE_URL = cpa_base_url.rstrip("/")

    cpa_token = str(getattr(args, "cpa_token", "") or "").strip()
    if cpa_token:
        CPA_TOKEN = cpa_token
    ...
    if mail_mode_raw:
        normalized_provider = normalize_mail_provider(mail_mode_raw)
        MAIL_PROVIDER_MODE = "config" if normalized_provider == "config" else normalized_provider

    mail_api = str(getattr(args, "mail_api", "") or "").strip()
    if mail_api:
        SELF_HOSTED_MESSAGES_API_URL = mail_api

    mail_domains = parse_mail_domains_csv(getattr(args, "mail_domains", ""))
    if mail_domains:
        SELF_HOSTED_MESSAGES_DOMAINS = tuple(mail_domains)


def resolve_loop_enabled(args: argparse.Namespace) -> bool:
    if args.loop is not None:
        return bool(args.loop)
    if getattr(args, "loop_mode", None) is not None:
        return bool(args.loop_mode)
    return bool(LOOP_MODE_ENABLED)


def resolve_clean_enabled(args: argparse.Namespace) -> bool:
    if getattr(args, "clean_mode", None) is not None:
        return bool(args.clean_mode)
    return bool(CLEAN_MODE_ENABLED)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="候选账号最小阈值（默认读取顶部常量 MAINTAINER_MIN_CANDIDATES）",
    )
    ...
    parser.add_argument("--loop", dest="loop", action="store_true", default=None, help="开启循环维护模式（按固定间隔重复执行清理+补号）")
    parser.add_argument("--once", dest="loop", action="store_false", help="强制单次执行，覆盖顶部 LOOP_MODE_ENABLED")
    parser.add_argument("--loop-mode", type=parse_cli_bool, default=None, help="是否开启 loop 模式：true/false；不传则使用默认值")
    parser.add_argument("--loop-interval", type=float, default=None, help="循环模式的检查间隔秒数（默认读取顶部常量 MAINTAINER_LOOP_INTERVAL_SECONDS）")
    parser.add_argument(
        "--token-json-dir",
        default="",
        help="token JSON 本地保存目录；传入后覆盖默认 output_tokens 目录，并自动启用本地保存",
    )
    parser.add_argument("--clean-files", dest="clean_mode", type=parse_cli_bool, default=None, help="是否先清理问题文件：true/false；不传则使用默认值")
    ...
    parser.add_argument(
        "--mail-mode",
        default="",
        help="邮箱模式：config/self_hosted_messages_api/self_hosted_mail_api/duckmail/tempmail_lol/cfmail/yyds_mail",
    )
...
def run_maintainer_once(args: argparse.Namespace, logger: logging.Logger, config_path: Path) -> int:
    ...
    try:
        conf = apply_runtime_overrides(conf, args)
    except Exception as e:
        logger.error("应用运行时参数失败: %s", e)
        return 2

    base_url = CPA_BASE_URL.rstrip("/")
    token = CPA_TOKEN.strip()
    target_type = str(pick_conf(conf, "clean", "target_type", default="codex") or "codex")
    clean_enabled = resolve_clean_enabled(args)

    if args.min_candidates is not None:
        min_candidates = int(args.min_candidates)
    else:
        min_candidates = int(MAINTAINER_MIN_CANDIDATES)
    ...
    if clean_enabled:
        ...
        logger.info("配置已关闭问题文件清理，跳过清理阶段。")
        try:
            ...
            logger.error("跳过清理后统计失败: %s", e)
            logger.info("=== 账号池自动维护结束（失败）===")
            return 4

        logger.info(
            "跳过清理统计: 总账号=%s, candidates=%s, 阈值=%s",
            total_after_clean,
            candidates_after_clean,
            min_candidates,
        )
...
def main() -> int:
    ...
    args = parse_args()
    apply_cli_global_overrides(args)
    config_path = Path(args.config).resolve()
    logger, _ = setup_logger(Path(args.log_dir).resolve())
    loop_enabled = resolve_loop_enabled(args)
    logger.info("=== 账号池自动维护开始（二合一）===")
    logger.info("配置文件: %s", config_path)
    logger.info("日志输出: 仅终端")
    logger.info("Flow Trace: 已关闭")
    logger.info("Loop 模式: %s", "开启" if loop_enabled else "关闭")
